...
Shader "Dodjoy/PostProcess/Bloom" {
	Properties {
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_Bloom ("Bloom (RGB)", 2D) = "black" {}
		_LumTex ("Luminance Tex (RGB)", 2D) = "black" {}		
	}
	...
	CGINCLUDE
		...
		fixed4 fragDebugBloom ( v2f_simple i ) : SV_Target
		{				
        	#if UNITY_UV_STARTS_AT_TOP
		...
		{	
        	...
        	#if UNITY_UV_STARTS_AT_TOP
			fixed3 bloom = tex2D(_Bloom, i.uv2).rgb;
			#else
			fixed3 bloom = tex2D(_Bloom, i.uv).rgb;			
			#endif
		...
		{				
			...
			fixed lumiance = tex2D(_LumTex, i.uv20).r;
			
			/*fixed4 color1 = tex2D (_MainTex, i.uv21);
			fixed4 color2 = tex2D (_MainTex, i.uv22);
			fixed4 color3 = tex2D (_MainTex, i.uv23);
					
			color.rgb = color.rgb + color1.rgb + color2.rgb + color3.rgb;
			return max(color/4 - THRESHHOLD, 0) * (lumiance * ONE_MINUS_THRESHHOLD_TIMES_INTENSITY * 4);
			*/
			
			color = color*lumiance;
			return max(color - THRESHHOLD, 0) * ONE_MINUS_THRESHHOLD_TIMES_INTENSITY;// * ONE_MINUS_THRESHHOLD_TIMES_INTENSITY;
		}
		...
		half4 fragBlur8 ( v2f_withBlurCoords8 i ) : SV_Target
		{
			...
			half2 coords = uv - netFilterWidth * 2.5;
